...
- specify if mitochondrial data should be used with `with_mito` (TRUE/FALSE)
- pick the genomes with `nuc_assemblies`
- set the reference with `id_ref` and `idRef`
- set the name of the outgroup (e.g. AMH_1a-0 or CBS432-0 or NCYC3947-0) with `id_ogroup` and `idOutgroup`
...
- choose one chunk between the one that runs phybwt2/distbwt2 and the one that copies the phybwt2/distbwt2 data from a previous independent run
...
- preparation of SNPs and indels from nucmer (chunks 1-7)
- production of the SNPs tree from nucmer with iqtree2 (chunk 8-9)
- classification of SNPs and all the other variants from pggb-vg (chunks 10-12)
- production of the SNPs tree from pggb-vg with iqtree2 (chunk 13)
- calculation of the cosine distances from nucmer and pggb-vg (chunk 14)
- production/copy of the reference-free cladogram and calculation of the cosine distances from distbwt2 (chunk 15-16)
- comparison of the distance matrices from nucmer and pggb-vg (17)
- production of the neighbour joining trees (chunk 18)
- comparison of the neighbour joining trees (chunk 19)
- comparison of the trees from iqtree2 and the neighbour joining algorithm (chunk 20)
- comparison of distbwt2 and pggb-odgi similarity (chunk 21)
...
We also make SNPs trees using iqtree2 but we use them only to check whether the trees produced in R are meaningful. The reason is that we cannot make a tree with iqtree2 using as input a distance matrix since iqtree2 accepts only alignments as input. Plus, we don't want all the modelling in iqtree2 to affect the comparison.
...
A full run will generate a tree of folders as the one reported in the main folder (`folder-tree.txt`).
...
First, we need to make a copy of the input fasta files and decide chose if we want to merge the nuclear genome with mitochondrial genome. This chunk creates the folder `gen/ass` with the formatted fasta genomes. For phased assemblies, the mitochondrial sequence is added only to the first haplotype. The fasta sequences are retrieved from a local repository `~/data/nano-assemblies-pansn-2024`.
...
### dev
### dirBase <- "/Users/Lorenzo/prog/graphs/sgp-eva/run-24"
...
dtChrom <- fread(pathRef, select = 1, header = F,
                 col.names = "Chr_id")[grep("^>", Chr_id)]
allChr <- sub(pattern = ">", replacement = "", x = dtChrom[, Chr_id])
...
for (indF in allFiles) {
  ...
  ###   all these dots have the same start coordinate (Pos_a1)
  ### - deletions are represented by dots in the ALT column (Allele_a2),
  ###   these dots have the same chromosome
  ###   and start coordinate (Chrom_a2:Pos_a2)
  ### e.g. two deletions may have the same start coordinate but may be in two 
  ### different chromosome; clearly this is generally true also for insertions
  ### but in this case the two insertions will be processed in two indC indexes
  ...
  for (indC in unique(dtNucIndels[["Chrom_a1"]])) {
    ...
    ### the genomic coordinate is unambiguously defined by "Pos_a1"
    ### since indC is the a chromosome of the reference
    ...
    ### debug dtNucChrIns <- dtNucChrIns[2290:2298,]
    ### here we need also the chromosome to
    ### unambiguously define the genomic coordinate
    dtNucChrIns[, Coord_a2 := paste(Chrom_a2, Pos_a2, sep = ":")]
    lsRleAlt <- rle(dtNucChrIns[["Coord_a2"]])
    ...
    if (nrow(dtNucChrIns) != 0) {
      ...
      for (indIns in 1:nIns) {
        strFRMR[indIns] <- dtNucChrIns[Coord_a2 == lsRleAlt$values[indIns],
                                       head(Strand_a1, 1)]
        strFRMQ[indIns] <- dtNucChrIns[Coord_a2 == lsRleAlt$values[indIns],
                                       head(Strand_a2, 1)]
        strCHRQ[indIns] <- dtNucChrIns[Coord_a2 == lsRleAlt$values[indIns],
                                       head(Chrom_a2, 1)]
        if (strFRMR[indIns] == 1) {
          numPOS[indIns] <- dtNucChrIns[Coord_a2 == lsRleAlt$values[indIns],
                                        head(Pos_a1, 1)] - 1
          ### the upstream aligned base and those deleted (from nucmer data)
          strAln <- lsVtFasta[[indC]][numPOS[indIns]]
          strDel <- paste(dtNucChrIns[Coord_a2 == lsRleAlt$values[indIns],
                                      Allele_a1], collapse = "")
          strREF[indIns] <- paste0(strAln, strDel)
          ### the position (in the reference) of the last
          ### upstream aligned base (with respect to the deletion)
          ### provides the ALT allele
          strALT[indIns] <- strAln
        } else {
          ### stop prints "Error: " in front of the text specified
          stop("antibodi, we found a reference reverse alignment.")
        }
      }
      vtCoord_a2 <- sapply(strsplit(x = lsRleAlt$values, split = ":"), "[[", 2)
      strSampleTags <- paste(strGT,
                           strFRMR,
                           strFRMQ,
                           strCHRQ,
...
for (indS in sampNames) {
  ...
  if (!file.exists(fileAln)) {
    cat("[", myName, "] ",
        ...
        "\n", sep = "")
    ### stop prints "Error: " in front of the text specified
    stop("alignment IDs do not match the IDs of the input SNP vcf file.")
  }
  dtAln <- fread(fileAln, sep = "\t", na.strings = "antani", tmpdir = baseDir)
...
### load the data
dtInVcf <- fread(fileInIndels, na.strings = "antani", tmpdir = dirBase)
colnames(dtInVcf)[1] <- "CHROM"
...
for (indS in sampNames) {
  ...
  dtAln <- fread(fileAln, sep = "\t", na.strings = "antani", tmpdir = dirBase)
  colnames(dtAln) <- hdAlnCoords
...
Here we remove invariant loci.

```{bash removes the invariant loci (nucmer)}
## settings -------------------------------------------------------------------
...
### output: multis-snps-genfix-noinv.vcf and multis-indels-genfix-noinv.vcf
all_files="${file_snps} ${file_indels}"
for ind_f in ${all_files}; do
  Rscript "${dir_base}/scr/no-invariant.R" "${ind_f}"
done

```

Now we can concatenate the SNPs and the indels since we will need them combined to build the small variants tree.
...
### input and output
file_snps="${dir_vcf}/multis-snps-genfix-noinv.vcf"
file_indels="${dir_vcf}/multis-indels-genfix-noinv.vcf"
file_out="${dir_vcf}/multis-smallv-genfix-noinv.vcf"
...
We can now build the tree from nucmer alignments with iqtree2.

```{bash makes the nucmer SNPs tree with iqtree2}
## settings -------------------------------------------------------------------
...
vcf_nuc=$(find "${dir_in}" -name "multis-snps-genfix-noinv.vcf")
...
### iqtree2 uses by default the first column sample as outgroup
phy_file=$(find . -name "*.min4.phy")
### the -bb option must be set at minimum 1000
iqtree2 -s "${phy_file}" -nt "${pll_threads}" -B 1000 -alrt 1000 \
-bnni -m "MFP+ASC" -seed 13735

### if any invariant site is still found 
### (god knows why)
### we need to run again iqtree2
file_log=$(find . -name "*min4.phy.log")
str_log=$(tail -1 "${file_log}" | grep "^ERROR" | cut -d ":" -f 1)
if [[ "${str_log}" == "ERROR" ]]; then
  id_phy=$(basename "${phy_file}" | cut -d "." -f 1)
  mv "${phy_file}" "${id_phy}.run-failed.min4.phy"
  phy_file=$(find . -name "*varsites.phy")
  iqtree2 -s "${phy_file}" -nt "${pll_threads}" -B 1000 -alrt 1000 \
  -bnni -m "MFP+ASC" -seed 13735
  
  ### save the number of invariant sites
  tail -2 "${file_log}" > "tail-run-failed.log"
  ### rename output to get rid of iqtree2 naming convention
  for ind_f in $(find . -name "*.varsites.phy*"); do
    new_name="${ind_f/.varsites.phy/}"
    mv -f "${ind_f}" "${new_name}"
  done
fi
...
Same as above but using only one chromosome for one tree.

```{bash makes the iqtree2 single-chromosome nucmer SNPs tree}
## settings -------------------------------------------------------------------
...
### clean and make the output folders
dir_phy_nuc="${dir_base}/phy/iqt/nuc-sc"
if [[ -d "${dir_phy_nuc}" ]]; then 
  rm -rf "${dir_phy_nuc}"
fi
mkdir -p "${dir_phy_nuc}"
...
### get all chromosomes
all_chrs=$(grep -v "^#" "${vcf_nuc}" | cut -f 1 | sort | uniq)

for ind_c in ${all_chrs}; do
  vcf_id=$(basename "${vcf_nuc}" | sed 's|.vcf$||')
  vcf_sc="${vcf_id}-${ind_c}.vcf"
  grep "^#" "${vcf_nuc}" > "${vcf_sc}"
  grep -w "^${ind_c}" "${vcf_nuc}" >> "${vcf_sc}"
  
  ### the outgroup is the first column of the phy file
  python3 "${dir_base}/scr/vcf2phylip.py" \
  -i "${vcf_sc}" --outgroup "${id_ogroup}"
  
  ### iqtree2 uses by default the first column sample as outgroup
  phy_file=$(find . -name "*-${ind_c}.min4.phy")
  ### the -bb option must be set at minimum 1000
  iqtree2 -s "${phy_file}" -nt "${pll_threads}" -B 1000 -alrt 1000 \
  -bnni -m "MFP" -seed 13735
  
  ### clean the single-chromosome vcf file
  rm -f "${vcf_sc}"
  
  ### if any invariant site is still found 
  ### (god knows why)
  ### we need to run again iqtree2
  file_log=$(find . -name "*${ind_c}.min4.phy.log")
  str_log=$(tail -1 "${file_log}" | grep "^ERROR" | cut -d ":" -f 1)
  if [[ "${str_log}" == "ERROR" ]]; then
    id_phy=$(basename "${phy_file}" | cut -d "." -f 1)
    mv "${phy_file}" "${id_phy}.run-failed.min4.phy"
    phy_file=$(find . -name "*${ind_c}*varsites.phy")
    iqtree2 -s "${phy_file}" -nt "${pll_threads}" -B 1000 -alrt 1000 \
    -bnni -m "MFP+ASC" -seed 13735
    ...
    tail -2 "${file_log}" > "tail-run-failed.log"
    ### rename output to get rid of iqtree2 naming convention
    for ind_f in $(find . -name "*${ind_c}*.varsites.phy*"); do
      new_name="${ind_f/.varsites.phy/}"
      mv -f "${ind_f}" "${new_name}"
    done
  fi
...
server_name="hulk"
### source folders
dir_server_base="prog/graphs/scspsj-hc"
run_pggb="run-1"
...
### dirBase <- "/Users/Lorenzo/prog/graphs/sgp-eva/run-17"
...
dir.create(dirSmall, recursive = T)
...
## correct empty GT values ----------------------------------------------------

### find them (vg version after 2023 produce empty GT entries) and replace them
nameCols <- names(dtVcf)
for (oneCol in nameCols) {
  set(dtVcf, i = which(dtVcf[[oneCol]] == ""), j = oneCol, value = "0")
}
...
### (needed for vg versions before 2024)
...
dtUnrolled <- fread(input = pathTmpUnrolled, header = T, sep = "\t",
                    na.strings = "antani", tmpdir = dirBase)
### cleaning
...
if (length(setDiff) != 0) {
  strWarnSetDiff <- paste0("[", myName, "] Some (other) small",
...
### make the vcf without invariant loci: "multis-msnps-sampfix-noinv.vcf"
Rscript "${dir_base}/scr/no-invariant.R" "multis-msnps-sampfix.vcf"
...
### make the vcf without invariant loci: "multis-smallv-sampfix-noinv.vcf"
Rscript "${dir_base}/scr/no-invariant.R" "multis-smallv-sampfix.vcf"
...
### make the vcf without invariant loci: "multis-indels-sampfix-noinv.vcf"
Rscript "${dir_base}/scr/no-invariant.R" "multis-indels-sampfix.vcf"

```

Then we can build the MNPs/SNPs tree from pggb-vg using iqtree2. This tree will tell how a sophisticated phylogenetic method compares to a raw tree construction method based on dissimilarity and clustering.

```{bash makes the pggb-vg SNPs tree with iqtree2}
## settings -------------------------------------------------------------------
...
vcf_msnps=$(find "${dir_in}" -name "multis-msnps-sampfix-noinv.vcf")

### move into output/working folder
cd "${dir_phy_psnp}"

### the outgroup is used to make the first column of the phy file
python3 "${dir_base}/scr/vcf2phylip.py" \
-i "${vcf_msnps}" --outgroup "${id_ogroup}"

### iqtree2 uses by default the first column sample as outgroup
phy_file=$(find . -name "*phy")
### the -bb option must be set at minimum 1000
iqtree2 -s "${phy_file}" -nt "${pll_threads}" -B 1000 -alrt 1000 \
-bnni -m "MFP+ASC" -seed 13735
...
### nucmer vcf files
pathNucSnps <- list.files(dirVcfNuc,
                          pattern = "multis-snps-genfix-noinv.vcf$",
                          full.names = T)
pathNucIndels <- list.files(dirVcfNuc,
                            pattern = "multis-indels-genfix-noinv.vcf$",
                            full.names = T)
pathNucSmallv <- list.files(dirVcfNuc,
                            pattern = "multis-smallv-genfix-noinv.vcf$",
                            full.names = T)
...
pathPosNorefOut <- file.path(dirDist, "dst-pos-nref.txt")
pathPosWithrefOut <- file.path(dirDist, "dst-pos-wref.txt")
...
for (indM in allDstMt) {
  ...
  ### save the phylogeny in tree (treefile for iqtree2) format
...
for (indT in 1:length(strClassesPvg)) {
  ...
  plot(nwkMrg,
       ...
       fsize = 1.5)
  tiplabels.cophylo(which = "left", pch = 18, cex = 3, col = "red",
                    frame = "none")
  tiplabels.cophylo(which = "right", pch = 18, cex = 3, col = "blue",
                    frame = "none")
  dev.off()
...
### set output
nameOut <- c("nuc-smallv-pos-nref.pdf")
pathOut <- file.path(dirOutput, nameOut)
### plot
pdf(file = pathOut, width = 24, height = 30)
plot(nwkMrg,
...
tiplabels.cophylo(which = "left", pch = 18, cex = 3, col = "red",
                  frame = "none")
tiplabels.cophylo(which = "right", pch = 18, cex = 3, col = "green4",
                  frame = "none")
dev.off()
...
### set output
nameOut <- c("pvg-smallv-pos-nref.pdf")
pathOut <- file.path(dirOutput, nameOut)
### plot
pdf(file = pathOut, width = 24, height = 30)
plot(nwkMrg,
     ...
     link.col = make.transparent("black", 0.25),
     fsize = 1.5)
tiplabels.cophylo(which = "left", pch = 18, cex = 3, col = "blue",
                  frame = "none")
tiplabels.cophylo(which = "right", pch = 18, cex = 3, col = "green4",
                  frame = "none")
dev.off()
...
plot(nwkMrg,
     ...
     link.col = make.transparent("black", 0.25),
     fsize = 1.5)
tiplabels.cophylo(which = "left", pch = 18, cex = 3, col = "purple",
                  frame = "none")
tiplabels.cophylo(which = "right", pch = 18, cex = 3, col = "green4",
                  frame = "none")
dev.off()
...
tiplabels.cophylo(which = "left", pch = 18, cex = 3, col = "purple",
                  frame = "none")
tiplabels.cophylo(which = "right", pch = 18, cex = 3, col = "red",
                  frame = "none")
dev.off()
...
Now we can compare the trees built with iqtree2 against the corresponding trees built with the neighbour joining algorithm.

```{r compares the iqtree2 trees vs the neighbour joining trees}
## header ---------------------------------------------------------------------
...
### write the header 
cat("variants", "\t" ,"GRF distance (iqtree2 vs nj-algorithm)", "\n",
...
### load the two newick files
namePvgIqt <- c("multis-msnps-sampfix-noinv.min4.phy.treefile")
...
plot(nwkMrg,
     ...
     link.col = make.transparent("black", 0.25),
     fsize = 1.5)
tiplabels.cophylo(which = "left", pch = 19, cex = 3, col = "blue",
                  frame = "none")
tiplabels.cophylo(which = "right", pch = 18, cex = 3, col = "blue",
                  frame = "none")
dev.off()
...
### load the two newick files
nameNucIqt <- c("multis-snps-genfix-noinv.min4.phy.treefile")
...
plot(nwkMrg,
     ...
     link.col = make.transparent("black", 0.25),
     fsize = 1.5)
tiplabels.cophylo(which = "left", pch = 19, cex = 3, col = "red",
                  frame = "none")
tiplabels.cophylo(which = "right", pch = 18, cex = 3, col = "red",
                  frame = "none")
dev.off()
...
```{r compares phybwt2 vs pggb-odgi similarity}
## header ---------------------------------------------------------------------
...
Here, we compare nucmer's trees, the whole-genome tree against all single-chromosome trees. All the trees are made with iqtree2. Remarkably, the whole-genome tree (on the left panel) may look different comparing two different plots (e.g. "wg-chrII.pdf" and "wg-chrX.pdf"). This totally normal since branches are rotated to minimise "movements" of the leaves of the two trees.

```{r compares nucmer wg tree vs nucmer sc trees}
## header ---------------------------------------------------------------------
...
### fixed settings
dirBase <- dirname(here())
### dev dirBase <- "/Users/Lorenzo/prog/graphs/sgp-eva/run-27"

### input folder
dirInput <- file.path(dirBase, "phy")
### output folder
dirOutput <- file.path(dirBase, "phy", "cmp-nuc-iqt")
unlink(dirOutput, recursive = T)
dir.create(dirOutput, showWarnings = F)
### output trees-distances file
pathTreeDstNucIqtWgSc <- file.path(dirBase, "phy", "cmp-nuc-iqt",
                                   "dst-nuc-iqt.txt")
unlink(pathTreeDstNucIqtWgSc)
### write the header 
cat("tree 1", "\t", "tree 2", "\t" ,"GRF distance", "\n", sep = "",
    file = pathTreeDstNucIqtWgSc)
...
## whole-genome vs single-chromosome ------------------------------------------

### load the whole-genome file
nameNucWg <- c("multis-snps-genfix-noinv.min4.phy.treefile")
pathNwkWg <- file.path(dirInput, "iqt", "nuc", nameNucWg)
nwkWg <- read.newick(pathNwkWg)

### single-chromosome files
dirInputSc <- file.path(dirInput,"iqt", "nuc-sc")
pathNwkSc <- list.files(path = dirInputSc,
                        pattern = "*min4.phy.treefile$",
                        full.names = T,
                        recursive = T)
### dev indS <- pathNwkSc[1]
for (indS in pathNwkSc) {
  ### load the single-chromosome file
  nwkSc <- read.newick(indS)
  strChr <- sub(".*-(chr[^.]+)\\..*", "\\1", basename(indS))
  
  ### concatenate
  nwkMrg <- cophylo(nwkWg, nwkSc, print = T, rotate = T)
  ### set output
  nameOut <- paste0("wg", "-", strChr, ".pdf")
  pathOut <- file.path(dirOutput, nameOut)
  ### plot
  pdf(file = pathOut, width = 24, height = 30)
  plot(nwkMrg,
  ...
  tiplabels.cophylo(which = "left", pch = 19, cex = 3, col = "red",
                    frame = "none")
  tiplabels.cophylo(which = "right", pch = 19, cex = 3, col = "red3",
                    frame = "none")
  dev.off()
  
  ### generalised Robinson–Foulds distance
  valTrDst <- TreeDistance(nwkWg, nwkSc)
  ### write to output
  nameT1 <- "wg"
  nameT2 <- strChr
  cat(nameT1, "\t", nameT2, "\t", valTrDst, "\n",
      sep = "", file = pathTreeDstNucIqtWgSc, append = T)
}

```

We can also compare apples (neighbour-joining trees) against oranges (iqtree2 phylogenies).

```{r compares apples (nja) and oranges (iqt)}
## header ---------------------------------------------------------------------
...
dirInput <- file.path(dirBase, "phy")
### output folder
dirOutput <- file.path(dirBase, "phy", "cmp-aao")
unlink(dirOutput, recursive = T)
dir.create(dirOutput, showWarnings = F)
### output trees-distances file
pathTreeDstAoo <- file.path(dirBase, "phy", "cmp-aao", "dst-aao.txt")
unlink(pathTreeDstAoo)
...
## nja-pos-nref vs iqt-nuc-snps -----------------------------------------------

### load the two newick files
pathNwkOne <- list.files(file.path(dirInput, "nja", "pos"),
                         pattern = "pos-nref.tree",
                         recursive = T, full.names = T)
pathNwkTwo <- list.files(file.path(dirInput, "iqt", "nuc"),
                         pattern = "multis-snps-genfix-noinv.*treefile",
                         recursive = T, full.names = T)
nwkOne <- read.newick(pathNwkOne)
nwkTwo <- read.newick(pathNwkTwo)
### concatenate
nwkMrg <- cophylo(nwkOne, nwkTwo, print = T, rotate = T)
### set output
nameOut <- paste0("nja-pos-nref", "-a-",
                  "iqt-nuc-snps", ".pdf")
pathOut <- file.path(dirOutput, nameOut)
### plot
pdf(file = pathOut, width = 24, height = 30)
plot(nwkMrg,
     ...
     link.col = make.transparent("black", 0.25),
     fsize = 1.5)
tiplabels.cophylo(which = "left", pch = 18, cex = 3, col = "green4",
                  frame = "none")
tiplabels.cophylo(which = "right", pch = 19, cex = 3, col = "red",
                  frame = "none")
dev.off()

### generalised Robinson–Foulds distance
valTrDst <- TreeDistance(nwkOne, nwkTwo)
### write to output
nameOne <- sub(pattern = "\\.tree$",
               replacement = "",
               x = basename(pathNwkOne))
nameOneFull <- paste0("nja-", nameOne)
nameTwoFull <- "iqt-nuc-snps"
cat(nameOneFull, "\t", nameTwoFull, "\t", valTrDst, "\n",
    sep = "", file = pathTreeDstAoo, append = T)

## nja-pos-nref vs iqt-pvg-msnps ----------------------------------------------

### load the two newick files
pathNwkOne <- list.files(file.path(dirInput, "nja", "pos"),
                         pattern = "pos-nref.tree",
                         recursive = T, full.names = T)
pathNwkTwo <- list.files(file.path(dirInput, "iqt", "pvg-msnps"),
                         pattern = "multis-msnps-sampfix-noinv.*treefile",
                         recursive = T, full.names = T)
nwkOne <- read.newick(pathNwkOne)
nwkTwo <- read.newick(pathNwkTwo)
### concatenate
...
tiplabels.cophylo(which = "left", pch = 18, cex = 3, col = "green4",
                  frame = "none")
tiplabels.cophylo(which = "right", pch = 19, cex = 3, col = "blue",
                  frame = "none")
dev.off()
...
nameOne <- sub(pattern = "\\.tree$",
               ...
               x = basename(pathNwkOne))
nameOneFull <- paste0("nja-", nameOne)
nameTwoFull <- "iqt-pvg-msnps"
cat(nameOneFull, "\t", nameTwoFull, "\t", valTrDst, "\n",
    sep = "", file = pathTreeDstAoo, append = T)

## nja-db2-nref vs iqt-nuc-snps -----------------------------------------------

### load the two newick files
pathNwkOne <- list.files(file.path(dirInput, "nja", "db2"),
                         pattern = "db2-nref.tree",
                         recursive = T, full.names = T)
pathNwkTwo <- list.files(file.path(dirInput, "iqt", "nuc"),
                         pattern = "multis-snps-genfix-noinv.*treefile",
                         recursive = T, full.names = T)
...
Since in the previous chunk we used two iqtree2 phylogenies that may be topologically identical we'd better quantify their similarity.

```{r compares the oranges}
## header ---------------------------------------------------------------------
...
dirInput <- file.path(dirBase, "phy")
### output folder
dirOutput <- file.path(dirBase, "phy", "cmp-iqt")
unlink(dirOutput, recursive = T)
dir.create(dirOutput, showWarnings = F)
### output trees-distances file
pathTreeDstAoo <- file.path(dirBase, "phy", "cmp-iqt", "dst-iqt.txt")
unlink(pathTreeDstAoo)
...
## iqt-nuc-snps vs iqt-pvg-msnps ----------------------------------------------

### load the two newick files
pathNwkOne <- list.files(file.path(dirInput, "iqt", "nuc"),
                         pattern = "multis-snps-genfix-noinv.*treefile",
                         recursive = T, full.names = T)
pathNwkTwo <- list.files(file.path(dirInput, "iqt", "pvg-msnps"),
                         pattern = "multis-msnps-sampfix-noinv.*treefile",
...
valTrDst <- TreeDistance(nwkOne, nwkTwo)
### write to output
nameOneFull <- "nuc-snps"
nameTwoFull <- "pvg-msnps"
cat(nameOneFull, "\t", nameTwoFull, "\t", valTrDst, "\n",
    sep = "", file = pathTreeDstAoo, append = T)
